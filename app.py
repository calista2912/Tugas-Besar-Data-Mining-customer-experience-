...
if uploaded_file:
    ...
    with tab3:
        ...
        if 'Retention_Status_Encoded' not in df.columns:
            ...
            # ==============================
            # Prediksi manual user input
            # ==============================
            st.markdown("---")
            st.subheader("🧪 Coba Prediksi Retensi (Manual Input)")

            with st.form("predict_form"):
                inputs = {}
                for col in X.columns:
                    val = st.number_input(f"Masukkan nilai untuk {col}", value=float(df[col].mean()))
                    inputs[col] = val
            ...
            if submitted:
                ...
                pred_result = model.predict(input_scaled)[0]
                proba_result = model.predict_proba(input_scaled)[0]

                label_map = {0: "❌ Churned", 1: "✅ Retained"}
                st.success(f"**Hasil Prediksi:** {label_map[pred_result]}")
                st.markdown(f"**Probabilitas:** Retained: `{proba_result[1]:.2%}` | Churned: `{proba_result[0]:.2%}`")
